...
的超时去掉了，现在不管 `_infer_once` 实际跑多久，都会等它真正跑完、
把模型真实算出来的距离值返回，不再有"超过预算就返回固定 10.0"这条路径
（模型加载失败/图像解码失败/ROI 内没有有效深度这些真正的错误路径不受
影响，那些没有真实值可给，仍然走兜底，这跟"时间"无关）。

**这是临时状态，不是最终方案**：榜单本身有 3 秒硬约束，去掉超时只是为了
先确认模型/后处理逻辑本身对不对——等看到几个真实预测值、确认量级合理
之后，超时保护必须加回来（哪怕最后调出来的真实推理时间稳定在 3 秒以内、
不需要保护了，也应该有意识地重新评估一次，不能放着不管）。
