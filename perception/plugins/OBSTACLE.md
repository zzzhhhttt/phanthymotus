...
## 2026-08-09 确认评测口径变化 + 真实硬件参数，新增本地评测脚本

**评测阈值从 F1@1m 改成 F1@2m**（用户确认）。仓库里所有更早的记录
（本文档前面几节、`plugins/obstacle_distance/eval.py` 里的
`_THRESHOLD_M = 1.0`）都是在 1m 口径下写的，历史数字不要直接拿来跟
2m 口径的新结果比——两个阈值下同一批预测的 F1 不是一回事。旧包的
`eval.py` 保留不动（已经不被 `main.py` 加载，只是历史参考），当前生效
的 `plugins/obstacle.py` 新增了 `perception/eval_obstacle.py` 作为
对应的本地自测工具，`_THRESHOLD_M = 2.0`，用法见文件内 docstring。

**室内/室外核心逻辑确认不改**：讨论过是否要（a）室内"最近点"退回最朴素
的 `min()`，（b）室外真正实现"3D OBB + 类别白名单"距离计算——两条都
明确保留现状不动：`min()` 已经在合成数据上验证过对单像素噪声不鲁棒
（见"稳健取值"一节），退回去是重新引入一个已经排查过的 bug；室外
"3D OBB + 类别白名单"物理上需要目标检测器（当前是纯单目深度图，没有
检测器，拿不到类别信息），真要做需要接入 `plugins/vop.py`
（YOLOv8-World），会让模型总体积远超 30MB 预算，不是这次"先跑通"范围
内的事，继续维持"ROI 内深度分位数近似最近障碍物距离"这个已知局限（见
下面小节），不冒充没有的能力。

**确认了真实运行硬件**：NVIDIA Jetson Orin 16GB（GPU/CPU 统一内存），
约 100 TOPS 算力，Ampere 架构（跟 RTX 3090 同代架构，但绝对算力差
很多），模型文件大小限制仍是单个 30MB 以下。这解释了之前排查 137/
超时问题时的几个判断为什么成立：
- Jetson 是统一内存，GPU 显存和系统内存是同一块物理内存池，`obstacle`
  插件默认强制 CPU、不碰 CUDA（见前面章节）这个方向是对的，不用再纠结
  "要不要用 GPU 省内存"这个问题——用不用 GPU，占用的都是同一块 16GB。
- Jetson Orin 的 CPU 核数（Cortex-A78AE，通常 8~12 核）远少于本地这台
  开发机（36 核），加上评测框架会并发起 10 个 obstacle 容器抢核，
  `intra_op_num_threads=1`（已经改过）这个选择是合理的，10 个容器 ×
  1 线程接近 8~12 个物理核的量级，不会像"每个容器都按36核自动探测"
  那样严重超订阅。
- 100 TOPS 是 Jetson Orin GPU（Tensor Core/DLA）的算力，当前完全没用到
  （CPU-only）——如果之后要往"用 GPU 换速度"这个方向优化，之前
  `OBSTACLE.md` 记录过量化模型的 `ConvInteger`/`MatMulInteger` 算子在
  普通 CUDAExecutionProvider 上支持不稳定，更合适的路径是 TensorRT EP
  （Jetson 原生支持得更好），但这是一次单独的、有风险的架构改动（TensorRT
  引擎构建/缓存、算子兼容性都要重新验证），不属于这次"先跑通"的范围，
  记在这里留给以后。
