...
**已知问题：延迟远超榜单预算，还没解决**——2026-08-05 真实评测显示
indoor/outdoor 单帧延迟分别约 20s / 40s，榜单要求 3s（`FPS=0.05`
`FPS=0.02`，本地小图测试量不出这个问题，只有在真实评测环境+真实分辨率
图片下才会暴露）。已经把 RANSAC 迭代次数从 150 降到 50（三倍算力），
但本地在 1600x900 真实分辨率合成数据上实测中值滤波+RANSAC 全部加起来
也就 0.6 秒左右，跟真实环境 20-40 秒的差距解释不了，大概率主要瓶颈在
别处（Jetson 实际 CPU 比这台开发机弱很多、depth_model 本身的 onnxruntime
CPU 推理、ROS2/Docker 开销），不是这次改的这几个函数。这个问题目前没
有阻塞打分（indoor 在延迟严重超标的情况下依然正常拿到了 F1=0.5），
但不代表可以放着不管，只是优先级排在"F1 从 0 变成正常值"之后，需要
之后单独在真实 Jetson 硬件上 profile 才能定位到具体瓶颈，本地这台
开发机器测不出来。
...
**outdoor 距离：直接用纵深值，不做换算**（2026-08-05 撤回过一次换算）。

榜单对 outdoor 的定义是"车头保险杠到最近障碍物表面的水平面（x-y 平面）
欧氏距离"，理论上跟相机纵深不是同一个量，之前确实按这个理论加过换算
（保险杠偏移 1.7m + 针孔相机模型反推横向偏移 X、`sqrt(X²+Z²)` 合成水平
距离，参考点/焦距都用的是 nuScenes 公开标定的估计值）。但 2026-08-05
真实评测结果证明这是负优化：outdoor RMSE 从换算前的 4.4231 涨到换算后
的 9.4513，几乎翻倍，F1@1m 依旧是 0.0000，没有任何正面效果。大概率是
横向换算依赖的焦距估计值不准，误差按"最近像素离图像中心的距离"成比例
放大到了每个 case 上，越换算越错。

现在改回跟 indoor 完全一样，直接用纵深值——这是目前 outdoor 实测 RMSE
最低的一版，也是唯一有真实数据支撑"没有让结果变差"的方法。经验教训：
在没有 GT 数据能验证的情况下，"理论上更对"的换算不代表"实测更准"，
尤其是换算本身依赖多个未经验证的估计值时，误差可能互相叠加而不是抵消。
今后如果要重新引入类似换算，应该先想办法拿到哪怕几个真实 GT 样本单独
验证效果，而不是一次性叠加多层没验证过的假设。
