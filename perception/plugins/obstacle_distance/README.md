...
- **indoor**：col 1/3~2/3、row 0~5/8 —— 跟榜单文档"6. 距离计算逻辑
  ...
  任意分辨率输入。
- **outdoor**：col 1/12~11/12、row 1/8~1（近似，尽量宽的裁剪，只避开
  广角镜头边缘畸变最严重的区域；2026-08-03 之前用过更窄的 col
  1/3~2/3、row 1/4~7/8，但实测一批 nuScenes CAM_FRONT case 出现
  F1@1m 精确等于 0.0000——怀疑窄 ROI 把车头正前方很近、画面里占比很大
  的障碍物直接裁在框外了，改宽之后还没有 case 级别的 GT 数据验证过，
  见下面 Limitations 和 `roi.py` 里 `outdoor_roi()` 的详细说明）。
